...
export function getCurrentContinuityState(): ContinuityState {
  ...
  if (exported.length === 0) {
    return {
      ...
      recommendedEvolution:
        "Establish the baseline. Introduce calm but authoritative pacing.",
    };
  }
  ...
  const recentIntensity = Math.min(
    100,
    Math.max(0, totalIntensity / recent.length),
  );

  // 2. Identity Persistence Evaluation
  let identityProfile: ContinuityState["identityProfile"] =
    "restrained_cinematic";
  ...
  if (recoveryDebt > 70) {
    recommendedEvolution =
      "User carries high recovery debt. Shift to decompression and calm mobility. Avoid aggressive hooks. Use 'recovery_editorial' identity.";
  } else if (recoveryDebt < 30 && recent.length > 2) {
    recommendedEvolution =
      "User is fresh. Build deeper intensity and focused drive. Introduce a challenging but steady pace. Lean into 'athletic_documentary'.";
  } else {
    recommendedEvolution =
      "Maintain continuity. Evolve familiarity slightly without breaking established pacing. Preserve 'calm_authoritative'.";
  }
