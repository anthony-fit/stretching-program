...
export function simulateViewerExperience(
  ...
  blueprint.scenes.forEach((scene) => {
    ...
    if (wps > 2.5) {
      ...
      fatigueAccumulation = Math.max(
        0,
        fatigueAccumulation - scene.duration * 0.5,
      ); // Recovery
    }
  ...
  const totalDuration = blueprint.scenes.reduce(
  ...
  let attentionRetention = Math.max(
    0,
    Math.min(100, (emotionalArc + silenceIntelligence) / 2),
  );
  ...
  if (context) {
    ...
    if (context.viewingEnvironment === "nighttime_recovery") {
      if (
        blueprint.pacingArc === "intervals" ||
        blueprint.pacingArc === "waves"
      ) {
        emotionalArc -= 40;
    ...
    if (
      ...
      if (blueprint.subtitleCadence === "relaxed") {
        ...
        risks.push(
          "Relaxed subtitle cadence risks losing viewers in silent environments.",
        );
      }
    }
  }
  ...
  if (cognitiveFatigueScore < 50) {
    risks.push(
      "High cognitive fatigue. Subtitle pressure or sustained intensity is too high.",
    );
  }
  cognitiveFatigueScore = Math.max(0, Math.min(100, cognitiveFatigueScore));

  const totalScore =
    silenceIntelligence * 0.3 +
    emotionalArc * 0.4 +
    cognitiveFatigueScore * 0.3;
