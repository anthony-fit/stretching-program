...
export const GOAL_JOURNEYS: GoalJourney[] = [
  {
    ...
    description: 'Restore foundational movement capacity and alleviate chronic stiffness.',
    milestones: [
      { id: 'm1', label: 'Pattern Initiation', workoutRequirement: 3, description: 'Stabilized basic decompression movement patterns.' },
      { id: 'm2', label: 'Joint Freedom', workoutRequirement: 8, description: 'Measurable expansion in hip and shoulder articulation.' },
      { id: 'm3', label: 'Mobility Mastery', workoutRequirement: 15, description: 'Full integration of restorative movement habits.' }
    ]
  },
  {
    id: 'posture-recovery',
    ...
    description: 'Counteract the effects of prolonged sitting and restore natural alignment.',
    milestones: [
      { id: 'p1', label: 'Alignment Awareness', workoutRequirement: 4, description: 'Improved proprioception of neutral spine position.' },
      { id: 'p2', label: 'Posterior Activation', workoutRequirement: 10, description: 'Strengthened core and glute support systems.' },
      { id: 'p3', label: 'Structural Resilience', workoutRequirement: 20, description: 'Natural posture maintained through long-duration sitting.' }
    ]
  },
  {
    ...
    description: 'Rebuild the bridge between isolated stretching and dynamic functional movement.',
    milestones: [
      { id: 'r1', label: 'Multi-planar Stability', workoutRequirement: 5, description: 'Controlled movement through all ranges of motion.' },
      { id: 'r2', label: 'Balance Threshold', workoutRequirement: 12, description: 'Significantly improved unilateral stability.' }
    ]
  },
  {
    ...
    description: 'Prepare the musculoskeletal system for higher intensity training loads.',
    milestones: [
      { id: 'pf1', label: 'Elastic Efficiency', workoutRequirement: 6, description: 'Improved force absorption and rebound capacity.' }
    ]
  },
  {
    id: 'desk-recovery',
    ...
    description: 'Targeted relief for the specific strains of modern desk-based work.',
    milestones: [
      { id: 'd1', label: 'Daily Decompression', workoutRequirement: 2, description: 'Established a consistent daily relief rhythm.' }
    ]
  }
];
...
export const PRESET_PROGRAMS: Program[] = [
  {
    ...
    phases: [
      { name: 'Decompress', type: 'Foundation', durationWeeks: 1, intensityTarget: 'Low', priorityFocus: ['Spine', 'Breathing'] },
      { name: 'Activate', type: 'Foundation', durationWeeks: 1, intensityTarget: 'Low', priorityFocus: ['Hips', 'Glutes'] },
      { name: 'Integrate', type: 'Building', durationWeeks: 2, intensityTarget: 'Medium', priorityFocus: ['Full Body Control'] }
    ],
    currentPhaseIndex: 0,
  ...
  },
  {
    ...
    phases: [
      { name: 'Technique', type: 'Foundation', durationWeeks: 2, intensityTarget: 'Medium', priorityFocus: ['Form', 'Stability'] },
      { name: 'Volume Build', type: 'Building', durationWeeks: 2, intensityTarget: 'Medium', priorityFocus: ['Hypertrophy'] },
      { name: 'Intensity Peak', type: 'Peak', durationWeeks: 2, intensityTarget: 'High', priorityFocus: ['Power'] }
    ],
    currentPhaseIndex: 0,
    startDate: new Date().toISOString()
  }
...
export function getDailyInspiration(progression: UserProgression, readiness: ReadinessScore): DailyInspiration {
  if (readiness.score < 60) {
    return {
      focus: 'System Restoration',
      ritualType: 'Recovery',
      message: 'Your physiological markers suggest a restorative approach. Prioritizing nervous system down-regulation today.'
    };
  }
  ...
  if (progression.momentumScore > 80) {
    return {
      ...
      ritualType: 'Mobility',
      message: 'Momentum is peak. Focusing on structural integrity to support your increased training volume.'
    };
  }
  ...
  return {
    ...
    message: 'Maintaining the rhythm. High-quality movement today keeps the momentum building.'
  };
}

export function calculateReadiness(progression: UserProgression): ReadinessScore {
  const avgIntensity = progression.recentIntensityExposure.length > 0 
    ? progression.recentIntensityExposure.reduce((a, b) => a + b, 0) / progression.recentIntensityExposure.length 
    : 0;
  ...
  // Inactivity penalty
  if (progression.consistencyStreak === 0 && progression.completedWorkoutCount > 0) {
    score -= 20;
  }
  ...
  const insights: MovementInsight[] = [
    {
      ...
      interpretation: score > 75 
        ...
        : score > 50 ? "Movement loading is balanced. Moderate recovery capacity." 
    ...
    },
    {
      ...
      status: loadStatus,
      interpretation: loadStatus === 'strained' 
        ? "Current intensity exceeds optimal movement window." 
        : loadStatus === 'optimal' ? "Load profile supports sustainable movement habits." : "Maintaining movement baseline."
    },
    {
      ...
      interpretation: progression.completedWorkoutCount > 10 
...
export function getTransformationNarrative(progression: UserProgression, journey: GoalJourney): TransformationNarrative {
  const milestone = [...journey.milestones].reverse().find(m => progression.completedWorkoutCount >= m.workoutRequirement);
  ...
  const nextMilestone = journey.milestones.find(m => m.workoutRequirement > progression.completedWorkoutCount);

  return {
    title: milestone.label,
    message: milestone.description + (nextMilestone ? ` Progressing toward ${nextMilestone.label}.` : ''),
    highlight: nextMilestone ? `${nextMilestone.workoutRequirement - progression.completedWorkoutCount} sessions to next level` : 'Journey Mastery Reached'
  };
}

export function getProgressionFromReports(reports: WorkoutReport[]): UserProgression {
  ...
  if (lastWorkout) {
    ...
    if (diffDays <= 2) { // 2 day grace period for streak
  ...
  if (reports.length >= 1) achievements.push({ id: 'first-session', title: 'First Step', description: 'Initiated the coaching journey.', dateAttained: reports[0].date, category: 'Phase' });
  if (reports.length >= 5) achievements.push({ id: 'consistency-5', title: 'Momentum Builder', description: 'Completed 5 sessions with discipline.', dateAttained: reports[4].date, category: 'Consistency' });
