...
import { getAdaptiveState, getStateAmbientClasses, getStateMotionMultiplier } from "../lib/runtime/adaptive";
...
const SOUNDTRACKS = {
  ...
  calm: { name: "Calm Atmosphere", url: "https://ia601402.us.archive.org/8/items/cd_relaxing-instrumentals_various-artists-antonio-carlos-jobim-art-/disc1/01.%20Novi%20Trio%20-%20Relaxing%20%5Bexcerpt%5D_sample.mp3" },
  focus: { name: "Deep Focus", url: "https://ia800408.us.archive.org/30/items/cd_ambient-1-music-for-airports_brian-eno/disc1/01.%20Brian%20Eno%20-%201%201_sample.mp3" },
  energy: { name: "Active Flow", url: "https://ia800109.us.archive.org/6/items/cd_energy_energy-energy-/disc1/02.%20Energy%20-%20Energy_sample.mp3" },
  minimal: { name: "Minimal Tech", url: "https://ia600100.us.archive.org/24/items/cd_minimal-music_steve-reich-terry-riley/disc1/01.%20Steve%20Reich%20-%20Piano%20Phase_sample.mp3" },
};
...
const STUDIO_TEMPLATES = [
  {
    ...
    description: "Gentle stretches to wake up the joints and boost circulation.",
    duration: 60,
    pacing: "natural",
    soundtrack: "calm",
  ...
  },
  {
    id: "desk",
    name: "Desk Reset",
    description: "Counteract 8 hours of sitting with targeted upper body relief.",
    duration: 60,
    pacing: "natural",
    soundtrack: "focus",
    exercises: ["Triceps Stretch", "Seated Forward Bend", "Neck Stretch", "Hip Opener"],
    focus: "Upper Body",
    level: "Beginner",
    type: "Mobility"
  },
...
const WORKOUT_TYPES = [
  { id: "Stretching", label: "Stretching", description: "Flexibility & Posture", icon: "Zap" },
  { id: "Mobility", label: "Mobility", description: "Recovery & Joint Health", icon: "Target" },
  { id: "Cardio", label: "Cardio", description: "Endurance & Fat Burn", icon: "Activity" },
  { id: "Strength", label: "Strength", description: "Muscle & Performance", icon: "Layers" },
  { id: "HIIT", label: "HIIT", description: "Intensity & Conditioning", icon: "Zap" },
  { id: "Balance", label: "Balance", description: "Stability & Control", icon: "Check" },
  { id: "Functional", label: "Functional", description: "Real-World Strength", icon: "Box" }
];
...
export default function VideoStudioPage() {
  ...
  const [exportState, setExportState] = useState<
    "idle" | "preparing" | "rendering" | "packaging" | "ready" | "review" | "failed"
  >("idle");
  ...
  const [wizardTab, setWizardTab] = useState<"custom" | "templates">("templates");
  ...
  const [framingMode, setFramingMode] = useState<"fit" | "focus" | "cinematic">("cinematic");
  const [transitionStyle, setTransitionStyle] = useState<"none" | "subtle" | "medium">("subtle");
  const [activeCreatorMode, setActiveCreatorMode] = useState<CreatorModeId | null>(null);
  ...
  const [subtitleStyle, setSubtitleStyle] = useState<"classic" | "bold" | "minimal">("classic");

  const applyCreatorMode = useCallback((modeId: CreatorModeId) => {
  ...
  const handleUndo = useCallback(() => {
    setUndoStack((prev) => {
      ...
      if (previousState) {
        ...
        const nextIndex = Math.min(activeItemIndex, Math.max(0, previousState.length - 1));
  ...
  useEffect(() => {
    Orchestrator.setScenes(storyboard.map(item => ({
  ...
  const [activeTab, setActiveTab] = useState<"assets" | "script" | "growth" | "progression" | "music" | "format">(
    "assets",
  );
  ...
  const pendingPreloads = useRef<Map<string, Promise<HTMLAudioElement>>>(new Map());
  ...
  const [currentProgression, setCurrentProgression] = useState<UserProgression | null>(null);
  const [activeProgram, setActiveProgram] = useState<Program | null>(null);
  const [currentJourneyId, setCurrentJourneyId] = useState<string>(GOAL_JOURNEYS[0].id);
  const [selectedStoryThemeId, setSelectedStoryThemeId] = useState<TransformationThemeId>('journey-update');
  const [showPhaseComplete, setShowPhaseComplete] = useState<string | null>(null);
  const [generationMessage, setGenerationMessage] = useState<string | null>(null);

  // Adaptive Presence Engine
  const adaptiveState = useMemo(() => getAdaptiveState(currentProgression, sessionDurationMinutes), [currentProgression, sessionDurationMinutes]);
  const ambientClasses = useMemo(() => getStateAmbientClasses(adaptiveState), [adaptiveState]);
  const motionMultiplier = useMemo(() => getStateMotionMultiplier(adaptiveState), [adaptiveState]);
  ...
  const [pacingMode, setPacingMode] = useState<"natural" | "energetic" | "relaxed">("natural");

  const applyPacingIntelligence = useCallback((items: StoryboardItem[]) => {
    ...
    return items.map((item, index) => {
      ...
      return {
        ...item,
        duration: Math.round((item.baseDuration || 10) * (durationWeight || 1))
      };
    });
  }, [pacingMode]);
  ...
  const [soundtrackPreset, setSoundtrackPreset] = useState<keyof typeof SOUNDTRACKS>("none");
    ...
    try {
      if (!audioContextRef.current) {
        ...
        if (AudioContextClass) {
          ...
          console.debug("[AudioEngine] Context initialized. State:", audioContextRef.current.state);
        }
      }
      ...
      if (ctx) {
        ...
        if (soundtrackAudioRef.current && !soundtrackSourceNodeRef.current) {
          try {
            soundtrackSourceNodeRef.current =
              ctx.createMediaElementSource(soundtrackAudioRef.current);

            soundtrackGainNodeRef.current = ctx.createGain();
            soundtrackGainNodeRef.current.gain.value = isSoundtrackEnabled ? soundtrackVolume : 0;

            soundtrackSourceNodeRef.current.connect(soundtrackGainNodeRef.current);
            soundtrackGainNodeRef.current.connect(ctx.destination);
            if (audioDestinationRef.current) {
              soundtrackGainNodeRef.current.connect(audioDestinationRef.current);
            }
            console.debug("[AudioEngine] Soundtrack connected via GainNode. Initial Gain:", soundtrackGainNodeRef.current.gain.value);
          } catch (e) {
            console.warn("[AudioEngine] Failed to connect soundtrack source:", e);
          }
        }
      }
      return ctx;
  ...
  useEffect(() => {
    ...
    if (track.url && isSoundtrackEnabled && !isPaused) {
      ...
      if (ctx?.state === "suspended") {
        ctx.resume().then(triggerPlay).catch((err) => {
  ...
  useEffect(() => {
    ...
    const handleMouseLeave = () => {
      isDown = false;
      el.classList.remove('cursor-grabbing');
      setTimeout(() => { isNavDraggingRef.current = false; }, 50);
    };
  ...
  useEffect(() => {
    if (navRailRef.current) {
      ...
      const activeBtn = el.querySelector(`#tab-btn-${activeTab}`) as HTMLElement;
      if (activeBtn) {
...
secondaryMuscles: ex.secondaryMuscles || [],
  ...
  const priorityContext = useMemo(() => {
    ...
    const activeCoach = COACH_PROFILES.find(c => c.id === wizardConfig.selectedCoachId) || COACH_PROFILES[0];
    ...
    if (currentProgression.momentumScore > 80 && readiness.score >= 75) {
      return {
        ...
        message: 'Momentum is peaking. Time to challenge your functional limits.',
    ...
    return {
      ...
      message: 'Consistency is the foundation of longevity. Maintain the rhythm.',
  ...
  const atmosphere = useMemo(() => {
    ...
    return calculateAtmosphere(currentProgression, readiness, activeCoach, currentPhase.name, currentJourney?.name);
  }, [currentProgression, activeProgram, wizardConfig.selectedCoachId, currentJourney]);
      ...
      if (saved) {
        ...
        if (parsed?.version === 1) {
          if (Array.isArray(parsed.storyboard) && parsed.storyboard.length > 0) {
            ...
            const validStoryboard = parsed.storyboard.filter((item: any) =>
              item && typeof item === 'object' && item.instanceId && item.name
            );
            setStoryboard(validStoryboard);
            if (!parsed.showWizard && validStoryboard.length > 0) setShowWizard(false);
          }
          ...
          if (parsed.socialCaptions) setSocialCaptions(parsed.socialCaptions);
              if (parsed.wizardConfig) {
                setWizardConfig({
                  ...
                  selectedCoachId: parsed.wizardConfig.selectedCoachId || COACH_PROFILES[0].id,
                });
              }
          if (parsed.aspectRatio) setAspectRatio(parsed.aspectRatio);
          if (parsed.vibe) setVibe(parsed.vibe);
          if (parsed.framingMode) setFramingMode(parsed.framingMode);
          if (parsed.transitionStyle) setTransitionStyle(parsed.transitionStyle);
  ...
  useEffect(() => {
    if (restored && !isLoading && location.state?.autoGenerate && !hasAutoGeneratedRef.current) {
      ...
      if (config) {
        hydratedConfig = {
          ...wizardConfig,
          duration: config.durationMinutes ? (config.durationMinutes * 60).toString() : wizardConfig.duration,
  ...
  const evolutionMessage = useMemo(() => {
    ...
    if (currentProgression.consistencyStreak > 0 && currentProgression.consistencyStreak % 3 === 0) {
  ...
  const generateRoutine = async (overrideConfig?: any) => {
    ...
    try {
      ...
      blueprint.scenes.forEach((scene: any) => {
        ...
        // Fallback intuitively if LLM halluci-mapped
        if (!exMatch) exMatch = exercises.find(ex => ex.name.toLowerCase() === scene.exerciseId.toLowerCase());
        if (!exMatch) exMatch = exercises[0];

        generatedItems.push({
          ...exMatch,
          duration: scene.duration || Math.max(10, Math.floor(totalSeconds / blueprint.scenes.length)),
          baseDuration: scene.duration || Math.max(10, Math.floor(totalSeconds / blueprint.scenes.length)),
          instanceId: generateId(),
      ...
      setWorkoutSummary({
         ...
         reasoning: blueprint.reasoning || "Autonomously mapped for optimal progression."
      });
  ...
  const handleGenerateAiScript = useCallback(
    async (items = storyboard, reasoning?: string, phase?: string, readiness?: string, momentum?: string, coachId?: string, userLevel?: string) => {
    ...
    [storyboard, wizardConfig.type, activeCreatorMode, wizardConfig.painPoints, wizardConfig.intensity, wizardConfig.coachingStyle, workoutSummary?.reasoning],
  );
  ...
  const applyTemplate = useCallback((template: any) => {
    ...
    template.exercises.forEach((name: string) => {
      // Direct name match
      let match = exercises.find(ex => ex.name?.toLowerCase() === name?.toLowerCase());

      // Fallback: Fuzzy search in name
      if (!match && name) {
        match = exercises.find(ex => ex.name?.toLowerCase().includes(name.toLowerCase()));
      }

      // Fallback: Category match
      if (!match && template.focus) {
        match = exercises.find(ex => ex.category?.toLowerCase().includes(template.focus.toLowerCase()));
      }
    ...
    if (matchedExercises.length < 3 && template.focus) {
      const pool = exercises.filter(ex => ex.category?.toLowerCase().includes(template.focus.toLowerCase()));
    ...
    const durationPerScene = Math.floor(template.duration / (matchedExercises.length || 1));
    ...
    const templateCreatorMode: CreatorModeId = template.type === "Mobility" || template.type === "Stretching" ? "wellness" : "shorts";
    ...
    setTimeout(() => {
      ...
      const phaseName = activeProgram?.phases[activeProgram.currentPhaseIndex].name;
      setGenerationMessage(`${template.name}: ${phaseName || 'Journey'} Established.`);
      setTimeout(() => setGenerationMessage(null), 5000);
  ...
  const handleGenerateSEOMetadata = async () => {
    ...
    try {
      ...
      const activeCoach = COACH_PROFILES.find(c => c.id === wizardConfig.selectedCoachId) || COACH_PROFILES[0];
      const storyTheme = TRANSFORMATION_THEMES.find(t => t.id === selectedStoryThemeId);
      const coachContext = {
  ...
  const removeFromStoryboard = useCallback((instanceId: string) => {
  ...
  const setItemDuration = useCallback((instanceId: string, duration: number) => {
    setStoryboard((prev) => {
      pushUndo(prev);
  ...
  const updateItemDuration = useCallback((instanceId: string, delta: number) => {
    setStoryboard((prev) => {
      pushUndo(prev);
      return prev.map((item) => {
        if (item.instanceId === instanceId) {
          const newDur = Math.max(5, Math.min(300, item.baseDuration + delta));
          return {
            ...item,
            duration: newDur,
            baseDuration: newDur,
  ...
  const downloadReadyExport = () => {
    ...
    // Assuming video/webm for now since we build standard blob
    const seoFileName = `${baseFileName}.webm`;
  ...
  const toggleRecording = async () => {
    ...
    for (const mimeType of mimeTypesToTry) {
      try {
        if (mimeType === "" || MediaRecorder.isTypeSupported(mimeType)) {
          ...
          console.debug(
            `[Export Status] Selected MIME type: ${selectedMimeType}`,
          );
    ...
    mr.onstop = () => {
      ...
      try {
        ...
        const blob = new Blob(recordedChunksRef.current, {
          type: finalMimeType.split(";")[0], // strip codecs for blob type
        });
        ...
        try {
          if (blob.size < 50_000) {
            console.warn("[Integrity Notice] Render artifact size is unusually small. Check timeline assets.");
          }
    ...
    const renderLoop = () => {
      ...
      const currProg = totalDuration > 0 ? Math.min(100, Math.floor((nextTimeState.globalTime / totalDuration) * 100)) : 0;
      ...
      const transitionSec = state.transitionStyle === "medium" ? 0.8 : state.transitionStyle === "subtle" ? 0.4 : 0;
      const isTransitioning = state.activeItemIndex > 0 && state.currentTime < transitionSec;
      const fadeProgress = isTransitioning ? (state.currentTime / transitionSec) : 1;

      const drawScene = (itemToDraw: any, progressTime: number, alpha: number) => {
        ...
        if (img && img.complete && img.naturalWidth > 0) {
          ...
          if (effectiveMode === "fit") {
            ...
            exportScale = baseScale * (startScale + progress * (endScale - startScale));
        ...
        const isMinimalUI = CREATOR_MODES.find(m => m.id === state.activeCreatorMode)?.minimalUI;
        ...
        if (!isMinimalUI) {
          ...
          const remaining = Math.max(0, (itemToDraw.duration || 0) - (progressTime || 0));
            ...
            if (words.length > 0) {
              ...
              if (activeSubtitle) {
                ...
                if (state.subtitleStyle === "bold") {
                    ...
                    if (state.subtitleStyle === "minimal") ctx.fillStyle = `rgba(10, 10, 10, ${subAlpha})`;
                }
        ...
        // Draw Hook Title
        if (state.activeItemIndex === 0 && state.currentTime < 2.5 && state.hookTitle) {
          const hookAlpha = Math.min(1, state.currentTime < 0.5 ? state.currentTime / 0.5 : (2.5 - state.currentTime) / 0.5);
          ctx.globalAlpha = hookAlpha * alpha;

          const theme = TRANSFORMATION_THEMES.find(t => t.id === selectedStoryThemeId);
          const hookValue = (theme ? `${theme.hookPrefix} ${state.hookTitle}` : state.hookTitle).toUpperCase();
        ...
        if (currentJourney) {
          ...
          // Coach watermark
          const coach = COACH_PROFILES.find(c => c.id === wizardConfig.selectedCoachId) || COACH_PROFILES[0];
          ctx.textAlign = "right";
          ctx.fillText(`COACH ${coach.name.toUpperCase()}`, cW - 50, 60);

          // Subtle progress bar at top
          const totalProgress = (state.activeItemIndex / state.storyboard.length) + (state.currentTime / (itemToDraw.duration * state.storyboard.length));
      ...
      if (isTransitioning) {
        ...
        drawScene(state.storyboard[state.activeItemIndex], state.currentTime, fadeProgress);
      } else {
        drawScene(state.storyboard[state.activeItemIndex], state.currentTime, 1);
      }
            ...
            >
              ...
              <div className="relative z-10">
                <div className="flex items-start justify-between mb-1">
                  ...
                  <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    >
                      ...
                      <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 opacity-0 group-hover/del:opacity-100 bg-charcoal text-cream text-[7px] font-black px-1 py-0.5 rounded tracking-tighter">DEL</div>
                    </button>
                  </div>
                </div>
                <p
                ...
                <div className="opacity-0 group-hover:opacity-100 transition-opacity mt-1">
                  <span className="text-[7px] font-black text-cream/30 uppercase tracking-widest bg-white/5 px-1.5 py-0.5 rounded-sm">Cmd+Z to Undo</span>
                </div>
              </div>
                  ...
                  <div className="flex items-center">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (!isNaN(val))
                          setItemDuration(item.instanceId, val);
                      }}
        ...
        {isGeneratingAi ? (
          ...
          <div className="py-24 text-center border-b border-white/5 opacity-60">
            ...
            <p className="text-[10px] text-white/50 font-medium max-w-[160px] mx-auto leading-relaxed">
              Build your movement sequence in the Library to trace coaching intelligence.
            </p>
          </div>
        ) : (
            ...
            {workoutSummary && (
              <div className="p-4 rounded-2xl border border-gold/30 bg-gold/5 relative overflow-hidden group">
                ...
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-[0.2em] text-gold">Coaching Intelligence</span>
                </div>

                <div className="space-y-4 relative z-10">
                  <div>
                    <h3 className="text-sm font-black uppercase tracking-tight text-white mb-1">{workoutSummary.primaryGoal}</h3>
                    <p className="text-[10px] text-white/60 leading-relaxed font-medium">{workoutSummary.reasoning}</p>
                  </div>

                  <div className="grid grid-cols-3 gap-2">
                    <div className="p-2 rounded-xl bg-white/5 border border-white/5">
                      <p className="text-[8px] font-black uppercase text-white/30 mb-1">Mobility</p>
                      <div className="h-1 bg-white/10 rounded-full overflow-hidden">
                        <div className="h-full bg-blue-400" style={{ width: `${workoutSummary.mobilityEmphasis}%` }} />
                      </div>
                      <p className="text-[9px] font-bold mt-1 text-blue-400">{workoutSummary.mobilityEmphasis}%</p>
                    </div>
                    <div className="p-2 rounded-xl bg-white/5 border border-white/5">
                      <p className="text-[8px] font-black uppercase text-white/30 mb-1">Strength</p>
                      <div className="h-1 bg-white/10 rounded-full overflow-hidden">
                        <div className="h-full bg-red-400" style={{ width: `${workoutSummary.strengthEmphasis}%` }} />
                      </div>
                      <p className="text-[9px] font-bold mt-1 text-red-400">{workoutSummary.strengthEmphasis}%</p>
                    </div>
                    <div className="p-2 rounded-xl bg-white/5 border border-white/5">
                      <p className="text-[8px] font-black uppercase text-white/30 mb-1">Cardio</p>
                      <div className="h-1 bg-white/10 rounded-full overflow-hidden">
                        <div className="h-full bg-green-400" style={{ width: `${workoutSummary.cardioEmphasis}%` }} />
                      </div>
                      <p className="text-[9px] font-bold mt-1 text-green-400">{workoutSummary.cardioEmphasis}%</p>
                    </div>
                  </div>

                  <div className="flex flex-wrap gap-2">
                    {workoutSummary.safetyNotes.map((note: string, i: number) => (
                      <div key={i} className="flex items-center gap-1.5 px-2 py-1 rounded-full bg-black/40 border border-white/5 text-[8px] font-bold text-white/70">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mb-2">
                      ...
                      {item.reason && (
                        <p className="text-[7px] font-black uppercase tracking-wider text-gold/60 mt-0.5">{item.reason}</p>
                      )}
                    </div>
  ...
  const completeWorkout = useCallback(() => {
    ...
    if (activeProgram) {
      const workoutsInProgram =
        workoutReports.length + 1;
      if (
        workoutsInProgram >= 10 &&
        activeProgram.currentPhaseIndex < activeProgram.phases.length - 1
      ) {
        setShowPhaseComplete(activeProgram.phases[activeProgram.currentPhaseIndex].name);
        setActiveProgram((prev) =>
          prev ? { ...prev, currentPhaseIndex: prev.currentPhaseIndex + 1 } : null,
        );
      }
    }

    showNotification("Workout saved to progression history!");
  }, [storyboard, wizardConfig.intensity, workoutSummary?.reasoning, activeProgram, workoutReports.length, showNotification]);
  ...
  const renderedProgressionTab = useMemo(() => {
    ...
    const theme = atmosphere || calculateAtmosphere(currentProgression, readiness, activeCoach, currentPhase.name);
        ...
        {workoutReports.length === 0 ? (
          <div className="space-y-6">
             ...
             <div className="p-7 border-b border-white/5 space-y-2 opacity-60">
                <h3 className="text-sm font-black uppercase tracking-widest text-white/50">Coach Initialization</h3>
                <p className="text-xs font-medium leading-relaxed max-w-sm">
                  The coaching intelligence module requires runtime data to synthesize progressions. Complete a workout to begin tracking readiness and adaptive pacing.
                </p>
             </div>

             <div className="p-5 space-y-4">
                <h4 className="text-[10px] font-black uppercase tracking-widest text-white/30">System Pipeline</h4>
                <div className="grid grid-cols-1 gap-4 opacity-50">
                   <div className="flex gap-4">
                      ...
                      <div>
                         <p className="text-[11px] font-bold text-white mb-0.5">Momentum Engine</p>
                         <p className="text-[9px] text-white/40 leading-relaxed">We track every session to build your movement profile. Consistency unlocks advanced coaching behaviors.</p>
                      </div>
                   </div>
                   <div className="flex gap-4">
                      <div className="w-8 h-8 rounded-xl bg-white/5 flex items-center justify-center shrink-0 border border-white/10">
                      ...
                      <div>
                         <p className="text-[11px] font-bold text-white mb-0.5">Customized Pathology</p>
                         <p className="text-[9px] text-white/40 leading-relaxed">By selecting pain points in the builder, our engine prioritizes corrective biomechanics for your specific needs.</p>
                      </div>
                   </div>
                </div>
                <button
          ...
          <div className="space-y-4">
            ...
            <div className="mb-2">
               ...
               <div className="p-6 rounded-[2.5rem] border border-white/10 bg-white/2 space-y-6 relative overflow-hidden group">
                  ...
                  <div className="flex justify-between items-start relative z-10">
                     <div className="space-y-1">
                        <h4 className="text-2xl font-serif italic text-white leading-tight">{currentJourney.name}</h4>
                        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-gold/80">{currentJourney.focus}</p>
                     </div>
                     <div className="px-4 py-1.5 rounded-full bg-white/5 border border-white/10 backdrop-blur-sm shadow-xl">
                        <span className="text-[8px] font-black uppercase tracking-widest text-white/60">
                          {currentProgression.completedWorkoutCount >= currentJourney.milestones[currentJourney.milestones.length-1].workoutRequirement ? 'Journey Complete' : 'Progressing'}
                        </span>
                     </div>
                  </div>
                     ...
                     <div className="space-y-4 relative z-10">
                        <div className="p-5 rounded-2xl bg-charcoal/40 border border-white/5 space-y-3 relative overflow-hidden group/narrative">
                           ...
                           <div className="flex items-center gap-2 relative z-10">
                              ...
                              <p className="text-[11px] font-bold text-white tracking-tight">{transformationNarrative.title}</p>
                           </div>
                           ...
                           <div className="flex items-center justify-between pt-3 border-t border-white/5 relative z-10">
                              <span className="text-[8px] font-black uppercase tracking-widest text-gold/60">{transformationNarrative.highlight}</span>
                              <div className="flex gap-2">
                                 {currentJourney.milestones.map((m, i) => (
                                    <div
                                      key={m.id}
                                      className={`w-2 h-2 rounded-full transition-all duration-700 ${
                                        currentProgression.completedWorkoutCount >= m.workoutRequirement ? 'bg-gold shadow-[0_0_8px_rgba(234,179,8,0.5)]' : 'bg-white/10'
                                      }`}
                                    />
                                 ))}
                              </div>
                  ...
                  <div className="grid grid-cols-2 gap-6 relative z-10 pt-2">
                     <div className="space-y-1">
                        <p className="text-[7px] text-white/30 uppercase font-black tracking-widest">Coaching Tone</p>
                        <p className="text-[11px] text-white font-medium">{currentJourney.coachingTone}</p>
                     </div>
                     <div className="space-y-1 text-right">
                        <p className="text-[7px] text-white/30 uppercase font-black tracking-widest">Movement Emphasis</p>
                        <p className="text-[11px] text-white font-medium italic">{currentJourney.movementEmphasis.slice(0, 2).join(' • ')}</p>
                     </div>
                  </div>
               </div>
               ...
               <div className="flex gap-2 mt-4 pb-2 overflow-x-auto no-scrollbar mask-fade-right">
                  {GOAL_JOURNEYS.map(j => (
                     ...
                     >
                        <p className={`text-[10px] font-bold mb-1 whitespace-nowrap ${currentJourneyId === j.id ? 'text-white' : ''}`}>{j.name}</p>
                        <p className="text-[7px] font-black uppercase tracking-widest opacity-60 whitespace-nowrap">{j.pacingStyle}</p>
                     </button>
                  ))}
               </div>
            </div>
            ...
            <motion.div
              ...
              style={{
                background: priorityContext.focus === 'recovery'
            ...
            >
              <div className="absolute -right-8 -top-8 opacity-10 group-hover:scale-110 transition-transform duration-1000">
                {priorityContext.icon && React.cloneElement(priorityContext.icon as any, { className: "w-48 h-48" })}
              </div>

              <div className="flex items-center justify-between mb-4">
                <div className={`px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest border ${
                  priorityContext.focus === 'recovery' ? 'bg-blue-500/20 text-blue-400 border-blue-500/30' : 'bg-gold/20 text-gold border-gold/30'
                }`}>
                  {priorityContext.focus} priority
                </div>
                <div className="flex -space-x-2">
                   {[1, 2, 3].map(i => (
                     <div key={i} className="w-5 h-5 rounded-full bg-white/10 border border-charcoal flex items-center justify-center text-[8px] font-bold">
                       {i}
                     </div>
                   ))}
                </div>
              ...
              <h3 className="text-2xl font-serif italic text-white mb-2 leading-tight">
                {priorityContext.focus === 'recovery' ? 'Nervous System Recovery' : priorityContext.focus === 'performance' ? 'Peak Output Potential' : 'Consistent Flow'}
              </h3>
              ...
              <div className="flex gap-3">
                <button
                  onClick={() => {
                    if (priorityContext.focus === 'recovery') startRitual('Recovery');
                  ...
                  className={`flex-1 py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all relative overflow-hidden group/btn ${
                    priorityContext.focus === 'recovery' ? 'bg-blue-500 text-white' : 'bg-gold text-charcoal'
                  }`}
                >
                  <span className="relative z-10">{priorityContext.primaryAction}</span>
            ...
            <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1 px-1">
               {priorityContext.focus === 'recovery' ? (
                 <>
                   <button onClick={() => startRitual('Recovery')} className="shrink-0 flex items-center gap-2 px-4 py-2.5 rounded-full bg-blue-500/10 border border-blue-500/20 text-[9px] font-black uppercase text-blue-400">
                     <Shield className="w-3 h-3"/> Neural Reset
                   </button>
                   <button onClick={() => setWizardTab('templates')} className="shrink-0 flex items-center gap-2 px-4 py-2.5 rounded-full bg-white/5 border border-white/10 text-[9px] font-black uppercase text-white/40">
                     <Calendar className="w-3 h-3"/> Slow Flow
                   </button>
                 </>
               ) : (
                 <>
                   <button onClick={() => setShowWizard(true)} className="shrink-0 flex items-center gap-2 px-4 py-2.5 rounded-full bg-gold/10 border border-gold/20 text-[9px] font-black uppercase text-gold">
                     <Zap className="w-3 h-3"/> Heavy Session
                   </button>
                   <button onClick={() => setActiveTab('growth')} className="shrink-0 flex items-center gap-2 px-4 py-2.5 rounded-full bg-white/5 border border-white/10 text-[9px] font-black uppercase text-white/40">
                     <Share2 className="w-3 h-3"/> Export Daily
                   </button>
                 </>
               )}
               <button onClick={() => setActiveTab('progression')} className="shrink-0 flex items-center gap-2 px-4 py-2.5 rounded-full bg-white/5 border border-white/10 text-[9px] font-black uppercase text-white/40">
            ...
            {priorityContext.focus === 'recovery' && (
              <div className="bg-red-500/5 border border-red-500/20 p-4 rounded-2xl flex gap-4 items-start">
                 ...
                 <div className="space-y-1">
                    <p className="text-[10px] font-black uppercase text-red-400">Physiological Load Alert</p>
                    <p className="text-[9px] text-red-500/60 leading-relaxed italic">Coach {activeCoach.name} has detected system strain. High-intensity routines are throttled to protect your baseline.</p>
                 </div>
              </div>
            )}
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 rounded-2xl border border-gold/30 bg-gold/5 relative overflow-hidden group">
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-[0.2em] text-gold">Active Program</span>
                    </div>
                    <span className="text-[9px] font-bold text-white/40 uppercase">Phase {activeProgram.currentPhaseIndex + 1}/{activeProgram.phases.length}</span>
                  </div>

                  <h3 className="text-sm font-black uppercase text-white mb-1">{activeProgram.name}</h3>
                  <p className="text-[10px] text-white/50 mb-4">{activeProgram.description}</p>

                  <div className="space-y-3">
                    <div className="flex items-center justify-between text-[9px] font-bold">
                      <span className="text-white/60">Current: {currentPhase.name}</span>
                      <span className="text-gold">{currentPhase.type}</span>
                    </div>
                    <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
                      <div className="h-full bg-gold transition-all duration-1000" style={{ width: `${(currentProgression.completedWorkoutCount % 10) * 10}%` }} />
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 rounded-xl border border-white/5 bg-white/5 flex flex-col items-center justify-center text-center">
                    <p className="text-[8px] font-black uppercase text-white/30 mb-2">Momentum</p>
                    <div className="relative w-12 h-12 flex items-center justify-center mb-1">
                      <svg className="w-full h-full -rotate-90">
                        ...
                        <circle
                          ...
                          className="text-gold"
                          strokeDasharray={126}
                          strokeDashoffset={126 - (126 * currentProgression.momentumScore) / 100}
                        />
                      </svg>
                      <div className="absolute flex flex-col items-center">
                        <Zap className="w-2.5 h-2.5 text-gold mb-0.5" />
                        <span className="text-[10px] font-black">{currentProgression.momentumScore}</span>
                      </div>
                    </div>
                    <p className="text-[8px] font-bold text-white/60 uppercase">Intensity Trend</p>
                  </div>

                  <div className="p-3 rounded-xl border border-white/5 bg-white/5 flex flex-col items-center justify-center text-center">
                    <p className="text-[8px] font-black uppercase text-white/30 mb-2">Readiness</p>
                    <div className="relative w-12 h-12 flex items-center justify-center mb-1">
                      <svg className="w-full h-full -rotate-90">
                        <circle cx="24" cy="24" r="20" fill="transparent" stroke="currentColor" strokeWidth="3" className="text-white/5" />
                        <circle
                          cx="24" cy="24" r="20" fill="transparent" stroke="currentColor" strokeWidth="3"
                          className={readiness.score > 70 ? "text-green-400" : readiness.score > 40 ? "text-gold" : "text-red-400"}
                          strokeDasharray={126}
                          strokeDashoffset={126 - (126 * readiness.score) / 100}
                        />
                      </svg>
                      <span className="absolute text-[10px] font-black">{readiness.score}</span>
                    </div>
                    <p className="text-[8px] font-bold text-white/60 uppercase">{readiness.recommendation}</p>
                  </div>
                </div>
              </div>
            ) : (
              <div className="p-8 border-b border-white/5 flex flex-col items-center justify-center text-center py-12 opacity-60">
                 <p className="text-[10px] font-black uppercase tracking-widest text-white/30">Phase 1: Foundation</p>
                 <p className="text-xs text-white/50 leading-relaxed max-w-[200px] mt-2">
                   Complete 3 sessions to unlock full performance telemetry and biometric history.
                 </p>
              </div>
            )}
            ...
            <div className="pt-4 pb-8 flex justify-center">
               <div className="flex items-center gap-3 px-4 py-2 opacity-50">
                  ...
                  <span className="text-[8px] font-black uppercase tracking-widest text-white/40">Coach {activeCoach.name} is observing your flow</span>
               </div>
            </div>
            ...
            >
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-0.5">
                  <h3 className="text-sm font-serif italic text-white leading-none">Coach {activeCoach.name}</h3>
                  <span
                ...
                <p className="text-[8px] text-white/50 leading-relaxed italic line-clamp-1">"{activeCoach.tagline}"</p>
              </div>
            </div>
          </div>
          ...
          >
             ...
             {evolutionMessage && (
               <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-gold/10 border border-gold/20">
                  ...
                  <span className="text-[9px] font-black uppercase text-gold tracking-tight">{evolutionMessage}</span>
               </div>
             )}
             ...
             <div className="grid grid-cols-2 gap-2 pt-1">
                ...
                >
                  ...
                  <span className="text-[8px] font-black uppercase tracking-tighter">Start Ritual</span>
                </button>
                <button
                ...
                >
                  ...
                  <span className="text-[8px] font-black uppercase tracking-tighter">Audio Focus</span>
                </button>
             </div>
          </div>
               ...
               {readiness.insights.map((insight, idx) => (
                 ...
                 >
                    <div className="flex items-center justify-between relative z-10">
                       <span className="text-[8px] font-black uppercase tracking-widest text-white/30">{insight.label}</span>
                       <div className={`w-1 h-1 rounded-full ${
                         insight.status === 'optimal' ? 'bg-green-400' :
                         insight.status === 'balanced' ? 'bg-gold' :
                         insight.status === 'strained' ? 'bg-red-400' : 'bg-blue-400'
                       } shadow-sm shadow-current`} />
                    </div>

                    <div className="flex items-baseline gap-1 relative z-10">
                       <span className="text-xl font-bold text-white tabular-nums">{insight.value}</span>
                       <span className={`text-[6px] font-black uppercase tracking-tighter ${
                          insight.status === 'optimal' ? 'text-green-400/60' :
                          insight.status === 'balanced' ? 'text-gold/60' :
                          insight.status === 'strained' ? 'text-red-400/60' : 'text-blue-400/60'
                       }`}>{insight.status}</span>
                    </div>
                    ...
                    <div className={`absolute bottom-0 right-0 w-12 h-12 blur-2xl opacity-5 rounded-full transition-all duration-700 ${
                       insight.status === 'optimal' ? 'bg-green-400' :
                       insight.status === 'balanced' ? 'bg-gold' :
                       insight.status === 'strained' ? 'bg-red-400' : 'bg-blue-400'
                    }`} />
          ...
          <div className="p-4 rounded-2xl bg-charcoal/50 border border-white/5 space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-white/70">Coaching Interpretation</span>
              </div>
            ...
            <div className="space-y-3">
              <div className="flex gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[7px] font-black uppercase tracking-widest text-white/30">Coach {activeCoach.name}</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2 pb-1 overflow-x-auto custom-scrollbar no-scrollbar">
                {currentProgression.achievements.map((ach) => (
                  <div key={ach.id} className="shrink-0 w-28 p-3 rounded-xl border border-gold/20 bg-gold/5 flex flex-col items-center text-center">
                    <div className="p-1.5 rounded-full bg-gold/10 text-gold mb-2">
                      <Award className="w-3 h-3" />
                    </div>
                    <p className="text-[8px] font-black uppercase text-white mb-0.5 line-clamp-1">{ach.title}</p>
                    <p className="text-[6px] text-white/40 leading-tight line-clamp-2">{ach.description}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="space-y-2">
            ...
            <div className="p-4 rounded-xl border border-white/5 bg-white/2 space-y-4">
               <div className="flex items-center justify-between">
                  <div>
                     <p className="text-[11px] font-bold text-white mb-0.5">Foundation Stability</p>
                     <p className="text-[8px] text-white/40">Assessment: {currentProgression.completedWorkoutCount > 5 ? 'High resolution' : 'Interpolating'}</p>
                  </div>
                  <div className="flex gap-1 items-end h-6">
                     {[40, 60, 55, 75, 85].map((h, i) => (
                       <div key={i} className="w-1 rounded-t-full bg-gold/40" style={{ height: `${h}%`, opacity: 0.2 + (i * 0.2) }} />
                     ))}
                  </div>
               </div>
               ...
               <div className="grid grid-cols-1 gap-2">
                  <div className="flex items-center gap-3">
                     ...
                     <p className="text-[9px] text-white/60">Consistency Pattern: <span className="text-white font-bold">{currentProgression.consistencyStreak > 2 ? 'Strengthening' : 'Establishing'}</span></p>
                  </div>
                  <div className="flex items-center gap-3">
                     <div className="w-1 h-1 rounded-full bg-blue-400" />
                     <p className="text-[9px] text-white/60">Movement Recovery: <span className="text-white font-bold">Trending Positive</span></p>
                  </div>
               </div>
            </div>
          </div>
            ...
            <div className="space-y-2">
              ...
              {readiness.score < 50 && (
                <div className="p-3 rounded-xl border border-red-500/30 bg-red-500/5 flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-[10px] font-bold text-red-500 mb-0.5">Overtraining Risk</p>
                    <p className="text-[9px] text-red-500/70 leading-relaxed">High-intensity stacking detected. Your recovery trend is declining; recommend active recovery sessions only.</p>
                  </div>
                </div>
              )}

              {/* Consistency Warning */}
              {currentProgression.consistencyStreak < 2 && currentProgression.completedWorkoutCount > 0 && (
                <div className="p-3 rounded-xl border border-gold/30 bg-gold/5 flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-[10px] font-bold text-gold mb-0.5">Momentum at Risk</p>
                    <p className="text-[9px] text-gold/70 leading-relaxed">Consistency has dipped recently. A short 5-minute movement snack today can restart your streak.</p>
                  </div>
                </div>
              )}
              ...
              <div className="p-3 rounded-xl border border-white/5 bg-white/5 flex items-start gap-3">
                ...
                <div>
                  <p className="text-[10px] font-bold text-white mb-0.5">Recovery Distribution</p>
              ...
              <div className="p-3 rounded-xl border border-white/5 bg-white/5 flex items-start gap-3">
                ...
                <div>
                  <p className="text-[10px] font-bold text-white mb-0.5">Progression Path</p>
                  <p className="text-[9px] text-white/50 leading-relaxed">
                    {currentProgression.completedWorkoutCount < 5
                      ...
                      : `Currently ${Math.round(
                          ((currentProgression.completedWorkoutCount % 10) / 10) * 100,
          ...
          {currentProgression.completedWorkoutCount >= 10 && (
            ...
            >
              ...
              <label className="text-[8px] font-black uppercase tracking-widest text-gold flex items-center gap-2">
                <Zap className="w-2.5 h-2.5" /> High-Resolution Movement Profile
              </label>
              <div className="grid grid-cols-2 gap-4 relative z-10">
                 <div className="space-y-1">
                    <p className="text-[7px] text-white/30 uppercase font-black">Force Dispersion</p>
                    <p className="text-[10px] text-white font-medium">Optimal Symmetry</p>
                 </div>
                 <div className="space-y-1">
                    <p className="text-[7px] text-white/30 uppercase font-black">Neural Efficiency</p>
                    <p className="text-[10px] text-white font-medium">92% Baseline</p>
                 </div>
              </div>
              <p className="text-[7px] text-white/40 leading-relaxed italic relative z-10 border-t border-white/5 pt-2">
                "Deep analysis indicates pattern mastery is reaching threshold. Coach is preparing advanced stability protocols for Phase 3."
              </p>
            </motion.div>
          )}
          ...
          <div className="p-4 rounded-xl border border-white/5 bg-white/2">
            <h4 className="text-[8px] font-black uppercase text-white/30 mb-3 tracking-widest">Switch Program</h4>
            <div className="space-y-2">
              {PRESET_PROGRAMS.map(p => (
                ...
                >
                  ...
                  <p className="text-[7px] text-white/40">{p.phases.length} Phases • {p.phases[0].intensityTarget} Intensity</p>
                </button>
              ))}
            </div>
          </div>
          ...
          {workoutReports.length > 0 && (
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                {workoutReports.slice(-3).reverse().map((report) => (
                  <div key={report.id} className="p-3 rounded-xl border border-white/5 bg-white/2 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="p-1.5 rounded-lg bg-white/5 text-white/40">
                        <Activity className="w-3 h-3" />
                      </div>
                      <div>
                        <p className="text-[10px] font-bold text-white">{new Date(report.date).toLocaleDateString()}</p>
                        <p className="text-[8px] text-white/40 uppercase font-black">{report.durationMinutes}m • {report.exercisesCompleted.length} Exercises</p>
                      </div>
                    </div>
                    <div className="flex gap-0.5">
                      {[...Array(5)].map((_, i) => (
                        <div key={i} className={`w-1 h-3 rounded-full ${i < report.perceivedExertion / 2 ? 'bg-gold' : 'bg-white/10'}`} />
                      ))}
                    </div>
                  </div>
                ))}
  ...
  }, [currentProgression, activeProgram, completeWorkout, storyboard.length, workoutReports, evolutionMessage, atmosphere, priorityContext, wizardConfig.selectedCoachId]);

  const TabButton = ({ id, label, icon: Icon, onClick, isActive, showGeneratingIndicator }: {
    id: string,
    label: string,
    icon?: any,
    onClick: () => void,
    ...
    >
      {Icon && <Icon className={`w-3.5 h-3.5 transition-transform duration-500 ${isActive ? "scale-110" : "group-hover:scale-110"}`} />}
    ...
    >
      ...
      {generationMessage && (
        <div className="fixed top-8 left-1/2 -translate-x-1/2 z-[200] w-full max-w-md px-4">
           ...
           >
              ...
              <p className="text-xs font-black uppercase tracking-tight">{generationMessage}</p>
           </motion.div>
        </div>
      )}
      {atmosphere && (
            ...
            <AnimatePresence>
              {isInitializingProtocol && (
                ...
                >
                   ...
                   <h2 className="text-3xl font-serif italic text-white mb-3">Orchestrating Workspace</h2>
                   <p className="text-[10px] uppercase tracking-[0.3em] font-medium text-white/40">Initializing Adaptive Logic</p>
                </motion.div>
              )}
            </AnimatePresence>
                     ...
                     >
                       ...
                       <div className="space-y-4">
                          ...
                          <div className="flex items-center gap-3">
                             ...
                             <span className="text-xs text-white/70">800+ Verified Assets Ready</span>
                          </div>
                       </div>
                     </motion.div>
                     ...
                     >
                       ...
                       <div className="p-4 bg-white/5 rounded-2xl border border-white/5 space-y-3">
                          <p className="text-xs text-white/80 leading-relaxed font-light">
                             “Morning routines are optimal right now. Momentum is stable. Atlas has prepared a flow.”
                          </p>
                ...
                <div className="max-w-5xl mx-auto space-y-12 flex flex-col min-h-full">
                  ...
                  >
                    <div className="flex items-center gap-3 mb-8">
                       ...
                       <h3 className="text-sm font-medium uppercase tracking-[0.2em] text-charcoal/50">Suggested Flows</h3>
                    </div>
                      ...
                      <AnimatePresence>
                        {STUDIO_TEMPLATES.map((tpl, idx) => (
                          <motion.button
                            ...
                            transition={{ ...transitionClasses.cinematic, delay: 0.4 + idx * 0.1 }}
                            key={tpl.id}
                          ...
                          >
                            ...
                            <div className="relative z-10 flex flex-col h-full min-h-[160px]">
                              <div className="flex items-center mb-6">
                                <div className={`w-12 h-12 rounded-[1rem] flex items-center justify-center transition-colors duration-500 ${idx === 0 ? 'bg-gold/10' : 'bg-charcoal/5 group-hover:bg-gold/10'}`}>
                                  {tpl.type === "Cardio" && <Activity className={`w-5 h-5 transition-colors ${idx === 0 ? 'text-gold' : 'text-charcoal/30 group-hover:text-gold'}`} />}
                                  {tpl.type === "Strength" && <Layers className={`w-5 h-5 transition-colors ${idx === 0 ? 'text-gold' : 'text-charcoal/30 group-hover:text-gold'}`} />}
                                  {(tpl.type === "Mobility" || tpl.type === "Stretching") && <Target className={`w-5 h-5 transition-colors ${idx === 0 ? 'text-gold' : 'text-charcoal/30 group-hover:text-gold'}`} />}
                                  {tpl.type === "HIIT" && <Zap className={`w-5 h-5 transition-colors ${idx === 0 ? 'text-gold' : 'text-charcoal/30 group-hover:text-gold'}`} />}
                                  {tpl.type === "Functional" && <Box className={`w-5 h-5 transition-colors ${idx === 0 ? 'text-gold' : 'text-charcoal/30 group-hover:text-gold'}`} />}
                                  {tpl.id === "desk" && <Monitor className={`w-5 h-5 transition-colors ${idx === 0 ? 'text-gold' : 'text-charcoal/30 group-hover:text-gold'}`} />}
                                </div>
                              </div>
                              <h3 className="font-serif italic text-2xl text-charcoal mb-2 group-hover:text-charcoal transition-colors">{tpl.name}</h3>
                              <p className="text-[11px] font-medium text-charcoal/40 leading-relaxed max-w-[90%] mt-auto">{tpl.description}</p>
                  ...
                  >
                    ...
                    <div className="space-y-4 md:space-y-6 mb-12">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                          {WORKOUT_TYPES.map((type) => (
                            ...
                            >
                              ...
                              >
                                {type.id === "Stretching" && <Zap className="w-3.5 h-3.5" />}
                                {type.id === "Mobility" && <Target className="w-3.5 h-3.5" />}
                                {type.id === "Cardio" && <Activity className="w-3.5 h-3.5" />}
                                {type.id === "Strength" && <Layers className="w-3.5 h-3.5" />}
                                {type.id === "HIIT" && <Zap className="w-3.5 h-3.5" />}
                                {type.id === "Balance" && <Check className="w-3.5 h-3.5" />}
                                {type.id === "Functional" && <Box className="w-3.5 h-3.5" />}
                              </div>
                              <div>
                      ...
                      <div className="space-y-3 md:space-y-6">
                        <div className="flex items-center justify-between">
                          <label className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.3em] text-charcoal/30 flex items-center gap-2">
                            <Clock className="w-3 h-3" /> Step 2: Session Length
                          </label>
                          <span className="text-[8px] font-black uppercase text-charcoal/40 italic">Micro-sessions (1m) build consistency</span>
                        </div>
                      ...
                      <div className="space-y-3 md:space-y-6">
                        <div className="flex items-center justify-between">
                          <label className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.3em] text-charcoal/30 flex items-center gap-2">
                             <Activity className="w-3 h-3" /> Step 3: Personalization
                          </label>
                          <span className="text-[8px] font-black uppercase text-blue-500/60">Biomechanical targeting enabled</span>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 md:gap-4">
                          <div className="space-y-2">
                             <label className="text-[9px] font-bold text-charcoal/50 uppercase tracking-widest">Pain Points</label>
                             <select
                                ...
                                onChange={(e) => setWizardConfig({ ...wizardConfig, painPoints: e.target.value ? e.target.value.split(',') : [] })}
                             >
                               <option value="">None</option>
                               <option value="Lower Back Pain">Lower Back Pain</option>
                               <option value="Neck Pain,Desk Posture">Desk Worker (Neck/Back)</option>
                          ...
                          <div className="space-y-2">
                             <label className="text-[9px] font-bold text-charcoal/50 uppercase tracking-widest">Target Muscles</label>
                             <select
                                className="w-full bg-cream/50 border border-charcoal/5 rounded-lg px-3 py-2.5 text-[10px] md:text-sm font-bold focus:outline-none focus:ring-2 focus:ring-gold/30 text-charcoal"
                                value={wizardConfig.targetMuscles[0] || ""}
                                onChange={(e) => setWizardConfig({ ...wizardConfig, targetMuscles: e.target.value ? [e.target.value] : [] })}
                             >
                                <option value="">Full Body</option>
                                <option value="Glutes">Glutes & Lower Body</option>
                                <option value="Core">Core & Abs</option>
                                <option value="Upper Body">Upper Body</option>
                                <option value="Pectoralis Major">Chest</option>
                                <option value="Erector Spinae">Spine/Back</option>
                             </select>
                          </div>

                          <div className="space-y-2">
                             <label className="text-[9px] font-bold text-charcoal/50 uppercase tracking-widest">Equipment</label>
                             <select
                                className="w-full bg-cream/50 border border-charcoal/5 rounded-lg px-3 py-2.5 text-[10px] md:text-sm font-bold focus:outline-none focus:ring-2 focus:ring-gold/30 text-charcoal"
                                value={wizardConfig.equipment.join(',')}
                                onChange={(e) => setWizardConfig({ ...wizardConfig, equipment: e.target.value.split(',') })}
                             >
                          ...
                          <div className="space-y-2">
                             <label className="text-[9px] font-bold text-charcoal/50 uppercase tracking-widest">Level</label>
                             <select
                                className="w-full bg-cream/50 border border-charcoal/5 rounded-lg px-3 py-2.5 text-[10px] md:text-sm font-bold focus:outline-none focus:ring-2 focus:ring-gold/30 text-charcoal"
                                value={wizardConfig.level}
                                onChange={(e) => setWizardConfig({ ...wizardConfig, level: e.target.value })}
                             >
                          ...
                          <div className="space-y-2">
                             <label className="text-[9px] font-bold text-charcoal/50 uppercase tracking-widest">Intensity</label>
                             <select
                                className="w-full bg-cream/50 border border-charcoal/5 rounded-lg px-3 py-2.5 text-[10px] md:text-sm font-bold focus:outline-none focus:ring-2 focus:ring-gold/30 text-charcoal"
                                value={wizardConfig.intensity}
                                onChange={(e) => setWizardConfig({ ...wizardConfig, intensity: e.target.value as any })}
                             >
                          ...
                          <div className="space-y-2">
                             <label className="text-[9px] font-bold text-charcoal/50 uppercase tracking-widest">Coaching Style</label>
                             <select
                                className="w-full bg-cream/50 border border-charcoal/5 rounded-lg px-3 py-2.5 text-[10px] md:text-sm font-bold focus:outline-none focus:ring-2 focus:ring-gold/30 text-charcoal"
                                value={wizardConfig.coachingStyle}
                                onChange={(e) => setWizardConfig({ ...wizardConfig, coachingStyle: e.target.value })}
                             >
                                <option value="Encouraging">Encouraging & Soft</option>
                                <option value="Direct">Direct & Professional</option>
                                <option value="Intense">Intense & Motivating</option>
                             </select>
                          </div>
                      ...
                      <div className="space-y-3 md:space-y-6">
                        <div className="flex items-center justify-between">
                          <label className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.3em] text-charcoal/30 flex items-center gap-2">
                            <Palette className="w-3 h-3" /> Step 4: Social Format
                          </label>
                          <span className="text-[8px] font-black uppercase text-charcoal/40">Optimizes cinematography for platform</span>
                        </div>
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                          {CREATOR_MODES.slice(0, 4).map((mode) => (
                            <button
                              key={mode.id}
                              onClick={() => setWizardConfig({ ...wizardConfig, creatorMode: mode.id })}
                            ...
                            >
                              <p className="text-[9px] font-black uppercase text-charcoal truncate">{mode.label}</p>
                              <p className="text-[7px] text-charcoal/40 font-bold leading-tight">{mode.description}</p>
                            </button>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="space-y-3 md:space-y-6 pb-6">
                        <div className="flex items-center justify-between">
                          <label className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.3em] text-charcoal/30 flex items-center gap-2">
                            <Mic2 className="w-3 h-3" /> Step 5: Coaching Identity
                          </label>
                          <span className="text-[8px] font-black uppercase text-gold">Coach maintains cross-session continuity</span>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                          {COACH_PROFILES.map((coach) => (
                            <button
                              key={coach.id}
                              onClick={() => setWizardConfig({ ...wizardConfig, selectedCoachId: coach.id })}
                            ...
                            >
                              <div className="relative z-10">
                                <div className="flex items-center justify-between mb-2">
                                  <p className={`text-[10px] font-black uppercase tracking-widest ${wizardConfig.selectedCoachId === coach.id ? "text-gold" : "text-charcoal/30"}`}>
                                    {coach.personality}
                                  </p>
                                  {wizardConfig.selectedCoachId === coach.id && (
                                ...
                                <h4 className="text-lg font-serif italic text-charcoal mb-0.5">{coach.name}</h4>
                                <p className="text-[9px] text-charcoal/50 leading-relaxed italic mb-2 line-clamp-1">"{coach.tagline}"</p>
                                <p className="text-[8px] text-charcoal/40 leading-relaxed font-medium line-clamp-2">{coach.philosophy}</p>
                              </div>
                  ...
                  <div className="flex flex-col md:flex-row gap-3 order-1 md:order-2 w-full md:w-auto">
                    ...
                    >
                      {dailyExports >= MAX_DAILY_EXPORTS ? (
                        ...
                        <>
                          Initialize Protocol <ArrowRight className="w-4 h-4 text-cream/50 group-hover:text-gold transition-colors group-hover:translate-x-1" />
                        </>
                      )}
                    </button>
                  </div>
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-x-hidden md:overflow-hidden md:min-h-0 min-w-0 w-full">
        ...
        <div className="w-full md:w-80 border-t md:border-t-0 md:border-r border-cream/10 flex flex-col bg-charcoal md:bg-charcoal/80 md:backdrop-blur-md order-2 md:order-1 md:h-auto overflow-x-hidden md:overflow-hidden shrink-0 md:min-h-0">
          ...
          <div ref={navRailRef} className="flex overflow-x-auto no-scrollbar snap-x snap-mandatory bg-[#1A1A1A] border-b border-white/5 relative z-10 shrink-0 select-none cursor-grab">
            <TabButton
              id="assets"
              label="Library"
              onClick={() => setActiveTab("assets")}
                ...
                {activeTab === "assets" && (
                  <div className="flex-1 flex flex-col md:h-full overflow-x-hidden md:overflow-hidden md:min-h-0">
                    <div className="px-6 pb-6 shrink-0 space-y-6">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <h1 className="text-xl font-serif italic tracking-tight font-light text-white/90">Studio Data</h1>
                        </div>
                        {storyboard.length > 0 && (
                          <button
                  ...
                  <div className="md:flex-1 overflow-x-hidden md:overflow-y-auto p-6 space-y-6 custom-scrollbar">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-[11px] font-black uppercase tracking-widest text-gold text-white/50">Instructional Intelligence</h3>
                      </div>
                      <button
                    ...
                    <div className="space-y-6">
                      <div className="flex items-center justify-between">
                         <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-[11px] font-black uppercase tracking-widest text-gold text-white/50">Sonic Atmosphere</h3>
                         </div>
                        <button
                          onClick={() => {
                            setupAudioContext();
                        ...
                        >
                          <div className={`absolute top-1 w-4 h-4 rounded-full bg-charcoal shadow-lg transition-all duration-500 ${isSoundtrackEnabled ? "left-6" : "left-1"}`} />
                        </button>
                      </div>

                      <div className="grid grid-cols-2 gap-3">
                        {(Object.keys(SOUNDTRACKS) as Array<keyof typeof SOUNDTRACKS>).map((preset) => (
                          <button
                            key={preset}
                            onClick={() => {
                              setupAudioContext();
                          ...
                          >
                            <p className={`text-[9px] font-black uppercase tracking-widest mb-1 ${soundtrackPreset === preset ? "text-gold" : "text-white/20"}`}>
                              {preset}
                            </p>
                            <p className="text-xs font-bold truncate group-hover:text-white transition-colors">{SOUNDTRACKS[preset].name}</p>
                      ...
                      <div className="space-y-4 px-2 pt-2">
                        <div className="flex justify-between items-center text-[9px] font-black uppercase tracking-wider text-white/30">
                          ...
                          <span className="text-white/60">{Math.round((soundtrackVolume || 0) * 100)}%</span>
                        </div>
                        <input
                          type="range"
                          min="0"
                    ...
                    <div className="pt-8 border-t border-white/5 space-y-6">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                           ...
                           <h3 className="text-[11px] font-black uppercase tracking-widest text-gold text-white/50">Pacing Engine</h3>
                        </div>
                        <div className="px-2 py-0.5 rounded-lg bg-gold/10 text-gold text-[8px] font-black uppercase border border-gold/10">Neural</div>
                      </div>
                      <div className="space-y-2">
                        {[
                          { id: "natural", label: "Structural Flow", desc: "Dynamic transitions based on neuro-response patterns" },
                          { id: "energetic", label: "Hyper-Paced", desc: "Shortened durations for active metabolic output" },
                          { id: "relaxed", label: "Restorative", desc: "Extended holds for parasympathetic activation" },
                        ].map((mode) => (
                          <button
                            key={mode.id}
                          ...
                          >
                            <div className="flex items-center justify-between mb-1.5">
                              <span className={`text-[10px] font-black uppercase tracking-widest ${pacingMode === mode.id ? "text-gold" : "text-white/50"}`}>
                                {mode.label}
                              </span>
                              {pacingMode === mode.id && <div className="w-1.5 h-1.5 rounded-full bg-gold" />}
                            </div>
                            <p className="text-[10px] text-white/30 leading-snug group-hover:text-white/50 transition-colors">{mode.desc}</p>
                          </button>
                        ))}
                      </div>
                    </div>
                  ...
                  <div className="md:flex-1 overflow-x-hidden md:overflow-y-auto p-6 space-y-8 custom-scrollbar">
                    <div className="space-y-6">
                      <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-[11px] font-black uppercase tracking-widest text-gold text-white/50">Visual Architecture</h3>
                      </div>

                      <div className="space-y-4">
                        <label className="text-[9px] font-black uppercase tracking-[0.2em] text-white/20 px-1">Cinematic Hook Title</label>
                        <div className="relative group">
                          <input
                            type="text"
                            value={hookTitle}
                      ...
                      <div className="space-y-4">
                         <div className="flex items-center justify-between px-1">
                            <label className="text-[9px] font-black uppercase tracking-[0.2em] text-white/20">Creator Presets</label>
                            <span className="text-[8px] font-bold text-gold/40">v1.2.0</span>
                         </div>
                        <div className="grid grid-cols-1 gap-2.5">
                          {CREATOR_MODES.map((mode) => (
                            ...
                            >
                              <div className="flex items-center justify-between mb-1.5">
                                <span className={`text-[10px] font-black uppercase tracking-widest ${activeCreatorMode === mode.id ? "text-gold" : "text-white/60"}`}>
                                  {mode.label}
                                </span>
                                {activeCreatorMode === mode.id && <Check className="w-3.5 h-3.5 text-gold" />}
                              </div>
                              <p className="text-[10px] text-white/30 leading-snug group-hover:text-white/50 transition-colors italic">{mode.description}</p>

                              <div className={`absolute left-0 top-0 w-1.5 h-full transition-all duration-500 ${activeCreatorMode === mode.id ? "bg-gold" : "bg-transparent group-hover:bg-white/10"}`} />
                            </button>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="pt-6 border-t border-white/5 space-y-5">
                        <div className="flex items-center justify-between px-1">
                          <span className="text-[9px] font-black uppercase tracking-widest text-white/20">Caption Generation</span>
                          <button
                            onClick={() => setSubtitlesEnabled(!subtitlesEnabled)}
                            className={`w-11 h-6 rounded-full transition-all duration-500 relative ${subtitlesEnabled ? "bg-gold" : "bg-white/5"}`}
                          >
                            <div className={`absolute top-1 w-4 h-4 rounded-full bg-charcoal transition-all duration-500 shadow-xl ${subtitlesEnabled ? "left-6" : "left-1"}`} />
                          </button>
                        </div>
                        ...
                        {subtitlesEnabled && (
                          ...
                          >
                            {(["classic", "bold", "minimal"] as const).map((style) => (
                              <button
                                key={style}
                  ...
                  <div className="md:flex-1 overflow-x-hidden md:overflow-y-auto p-6 space-y-8 custom-scrollbar">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-[11px] font-black uppercase tracking-widest text-gold text-white/50">Ecosystem Amplification</h3>
                      </div>
                      <button
                      ...
                      >
                        <Wand2 className={`w-4 h-4 ${isGeneratingSEO ? "animate-spin" : ""}`} />
                      </button>
                    </div>

                    <div className="space-y-4">
                      <label className="text-[9px] font-black uppercase tracking-[0.2em] text-white/20 px-1">Narrative Framing</label>
                      <div className="grid grid-cols-2 gap-2.5">
                        {TRANSFORMATION_THEMES.map(theme => (
                          ...
                          >
                            <p className={`text-[10px] font-black uppercase tracking-wider mb-1.5 ${selectedStoryThemeId === theme.id ? 'text-gold' : 'text-white/40'}`}>
                              {theme.label}
                            </p>
                      ...
                      {selectedStoryThemeId && (
                        <div className="p-4 rounded-2xl bg-white/[0.02] border border-white/5 backdrop-blur-md shadow-lg">
                           <p className="text-[10px] text-white/40 leading-relaxed italic font-medium">
                              <span className="text-gold/80 font-black uppercase mr-2 tracking-widest text-[8px]">Intelligence Directive:</span>
                              Prioritizing {TRANSFORMATION_THEMES.find(t => t.id === selectedStoryThemeId)?.narrativeFocus} within all generated metadata and social assets.
                           </p>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="pt-6 border-t border-white/5">
                      {isGeneratingSEO ? (
                        <div className="space-y-6 py-12 text-center">
                          <div className="space-y-2">
                            <p className="text-[11px] font-black uppercase tracking-[0.3em] text-white/40">Synthesis in Progress</p>
                            <p className="text-[10px] text-white/20 italic font-medium">Calibrating SEO vectors to movement patterns...</p>
                          </div>
                        </div>
                      ) : storyboard.length === 0 ? (
                        <div className="py-24 text-center space-y-4 opacity-60">
                          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-white/30">Empty Repository</p>
                          <p className="text-[10px] text-white/50 font-medium max-w-[140px] mx-auto leading-relaxed">Build a ritual in the Library to initialize intelligence vectors.</p>
                        </div>
                        ...
                        <div className="space-y-6">
                          <div className="p-5 rounded-[2rem] border border-white/5 bg-white/[0.02] hover:bg-white/[0.04] transition-all group/seo">
                            <div className="flex items-center justify-between mb-4">
                              <h4 className="text-[9px] font-black uppercase tracking-[0.2em] text-white/30">Movement SEO Vectors</h4>
                              <button
                                onClick={() => navigator.clipboard.writeText(seoMetadata.metaDescription)}
                            ...
                            <div className="space-y-4">
                              <div>
                                <span className="text-[8px] font-black uppercase text-gold/60 tracking-widest">Global Title Path</span>
                                <p className="text-sm font-serif italic text-white mt-1 leading-tight group-hover/seo:text-glow transition-all">{seoMetadata.youtubeTitle}</p>
                              </div>
                              <div>
                                <span className="text-[8px] font-black uppercase text-gold/60 tracking-widest">Structural Description</span>
                                <p className="text-[10px] text-white/50 leading-relaxed mt-1.5 font-medium">{seoMetadata.metaDescription}</p>
                              </div>
                              <div>
                                <span className="text-[8px] font-black uppercase text-gold/60 tracking-widest">Visibility Anchors</span>
                                <p className="text-[10px] text-gold/60 mt-1.5 font-bold tracking-tight">
                                  {seoMetadata.hashtags?.map((h: string) => "#" + h).join(" ")}
                                </p>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="p-5 rounded-[2rem] border border-white/5 bg-white/[0.02] hover:bg-white/[0.04] transition-all">
                             <div className="flex items-center justify-between mb-4">
                                <h4 className="text-[9px] font-black uppercase tracking-[0.2em] text-white/30">Amplification Captions</h4>
                             </div>
                             <div className="space-y-6">
                                {[
                                  { id: 'tiktok', label: 'TikTok Neural', color: '#ff0050', content: socialCaptions.tiktokCaption },
                                  { id: 'insta', label: 'Instagram Aesthetic', color: '#C13584', content: socialCaptions.instagramCaption },
                                  { id: 'shorts', label: 'Short-Form Narrative', color: '#FF0000', content: socialCaptions.youtubeShortsCaption }
                                ].map((plat, idx) => (
                                  <div key={plat.id} className={`group/plat ${idx !== 0 ? 'pt-6 border-t border-white/5' : ''}`}>
                                    <div className="flex justify-between items-center mb-3">
                                      <span className="text-[8px] font-black uppercase tracking-widest flex items-center gap-2" style={{ color: plat.color }}>
                                        <div className="w-1 h-1 rounded-full" style={{ backgroundColor: plat.color }} />
                                        {plat.label}
                                      </span>
                                      <button
                                        onClick={() => navigator.clipboard.writeText(plat.content)}
                                    ...
                                    <p className="text-[10px] text-white/60 leading-relaxed whitespace-pre-wrap font-medium">{plat.content}</p>
                                  </div>
                                ))}
                             </div>
                          </div>
                        ...
                        <div className="py-20 text-center space-y-6">
                           ...
                           <div className="space-y-2">
                              <p className="text-[11px] font-black uppercase tracking-[0.3em] text-gold/60">Ready for Synthesis</p>
                              <p className="text-[10px] text-white/20 font-bold max-w-[180px] mx-auto italic">Activate AI metadata processing to unlock platform-specific growth hooks.</p>
                           </div>
                           <button
                             onClick={() => handleGenerateSEOMetadata()}
        ...
        <div className="flex-1 flex flex-col relative bg-[#0a0a0a] order-1 md:order-2 md:h-auto overflow-x-hidden md:overflow-hidden shrink-0 min-w-0 md:min-h-0">
          ...
          <div className="sticky top-0 z-50 h-14 md:h-20 border-b border-cream/10 flex items-center justify-between px-3 md:px-8 bg-charcoal/80 overflow-x-auto no-scrollbar shrink-0 backdrop-blur-xl">
            <div className="flex items-center gap-3 md:gap-6 flex-shrink-0">
              <div className="flex items-center bg-cream/5 rounded-lg p-0.5 md:p-1 border border-cream/10">
                {[
                  { id: "9:16", icon: Smartphone, tip: "Portrait (TikTok/Shorts)" },
              ...
              <div className="flex items-center bg-cream/5 rounded-lg p-0.5 md:p-1 border border-cream/10">
                {[
                  { id: "fit", icon: LayoutGrid, full: "fit", tip: "Safe margins" },
                  { id: "focus", icon: Box, full: "focus", tip: "Center focus" },
                  { id: "cine", icon: Play, full: "cinematic", tip: "Dynamic movement" },
                ].map((f) => (
                  <button
                    key={f.full}
              ...
              <div className="flex flex-col items-end gap-1">
                {exportState === "ready" || exportState === "review" ? (
                  ...
                  >
                    ...
                    {(exportState === "rendering" || exportState === "packaging") && (
                       <div
                         className="absolute inset-0 bg-white/20 transition-all duration-300 pointer-events-none"
                         style={{ width: `${renderProgress}%`, transformOrigin: 'left' }}
                       />
                    )}
                ...
                {storyboard.length > 0 && exportState !== "review" && (
                  <p className="text-[8px] md:text-[9px] text-charcoal/40 font-bold max-w-[240px] text-right pt-2 leading-relaxed uppercase tracking-widest hidden md:block">
                    {exportState === "idle" ? "Rendering involves strict deterministic synchronization. Duration depends on video complexity."
                      : exportState === "rendering" ? "Deterministic orchestration engaged. Workspace locked."
                      : exportState === "ready" ? "FINAL COMPOSITION AVAILABLE"
                      : ""}
                  </p>
                )}
              </div>
          ...
          <div className="flex-1 flex items-center justify-center p-4 md:p-12 bg-pattern overflow-hidden min-h-[300px] min-w-0">
            ...
            >
              ...
              <svg
                viewBox={aspectRatio === "9:16" ? "0 0 9 16" : aspectRatio === "16:9" ? "0 0 16 9" : "0 0 1 1"}
                className={`opacity-0 pointer-events-none block transition-all duration-700 ${
                  aspectRatio === "16:9" ? "w-[1000px] max-w-full max-h-full h-auto" :
                  aspectRatio === "9:16" ? "h-[750px] max-h-full max-w-full w-auto" :
                ...
                <div className="absolute inset-0">
                  <AnimatePresence initial={false}>
                    <motion.div
                      ...
                      initial={{ opacity: transitionStyle === "none" ? 1 : 0 }}
                      animate={{ opacity: 1 }}
                      exit={{ opacity: transitionStyle === "none" ? 1 : 0 }}
                      transition={{
                        duration: transitionStyle === "medium" ? 0.8 : transitionStyle === "subtle" ? 0.4 : 0,
                        ease: "easeInOut"
                      }}
                  ...
                  <AnimatePresence mode="wait">
                    ...
                    >
                      ...
                      {!CREATOR_MODES.find(m => m.id === activeCreatorMode)?.minimalUI && (
                          ...
                          <div className="space-y-1 md:space-y-2 mix-blend-darken">
                            ...
                            {vibe !== "minimal" && (
                              <p className="text-charcoal/30 text-[8px] md:text-[10px] font-black uppercase tracking-[0.3em]">
                                {storyboard[activeItemIndex].category} • Module
                              </p>
                            )}
                          </div>
                          ...
                          <div className="space-y-2 md:space-y-4 max-w-[50%]">
                            <div className="space-y-1">
                              <p className="text-charcoal/40 text-[7px] md:text-[9px] font-black uppercase tracking-[0.3em] flex items-center gap-1.5">
                                <span className="w-1 h-1 bg-gold rounded-full" /> Instructions
                              </p>
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center text-charcoal/30 p-6 md:p-12 text-center bg-cream/80 backdrop-blur-sm z-0">
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      {[
                        { step: "1", text: "Add routines from the Library", icon: Plus },
                        { step: "2", text: "Review AI Narration Scripts", icon: Mic2 },
                        { step: "3", text: "Export for social media", icon: Download },
                      ].map((item) => (
                        <div key={item.step} className="flex items-center gap-4 bg-white/50 p-4 rounded-2xl border border-charcoal/5 shadow-sm">
            ...
            {exportState !== "idle" && exportState !== "failed" && (
              <div className="absolute inset-0 bg-charcoal/80 backdrop-blur-sm z-50 flex items-center justify-center p-8 transition-all duration-500">
                 <div className="flex flex-col items-center gap-3">
                    <Monitor className={`w-8 h-8 md:w-10 md:h-10 text-white/20 ${exportState === "review" || exportState === "ready" ? "" : "animate-pulse"}`} />
                    <p className="text-[10px] md:text-xs font-black uppercase tracking-[0.3em] text-white/40">
                      {exportState === "review" || exportState === "ready" ? "Master Composition Authoritative" : "Temporal State Locked"}
                    </p>
                    <p className="text-[9px] md:text-[10px] text-white/30 font-medium">
                      {exportState === "review" || exportState === "ready" ? "You are previewing the final delivery package." : "Finalizing orchestration master timeline."}
                    </p>
                 </div>
              </div>
            )}
              ...
              <div className="flex items-center gap-3 md:gap-6">
                <button
                  onClick={() => {
                    const ctx = setupAudioContext();
                    if (ctx?.state === "suspended") {
                      ctx.resume().then(() => isPaused ? Orchestrator.play() : Orchestrator.pause());
                    } else {
                      isPaused ? Orchestrator.play() : Orchestrator.pause();
                    }
                  }}
          ...
          >
            ...
            >
              ...
              <div className="flex flex-col items-center gap-4">
                ...
                <div className="space-y-1">
                  <p className="text-[10px] font-black uppercase tracking-[0.3em] text-gold">Phase Complete</p>
                  <h2 className="text-3xl font-serif italic text-white">{showPhaseComplete}</h2>
                </div>
              </div>

              <div className="p-6 rounded-2xl bg-white/5 border border-white/5 text-left space-y-4">
                <p className="text-[10px] uppercase font-black tracking-widest text-white/30">Progression Summary</p>
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <p className="text-[8px] font-bold text-white/40 uppercase">Total Sessions</p>
                    <p className="text-xl font-black text-gold">{currentProgression?.completedWorkoutCount}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-[8px] font-bold text-white/40 uppercase">Max Readiness</p>
                    <p className="text-xl font-black text-gold">94%</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-[8px] font-bold text-white/40 uppercase">Best Streak</p>
                    <p className="text-xl font-black text-gold">{currentProgression?.consistencyStreak} Days</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-[8px] font-bold text-white/40 uppercase">Focus Mastery</p>
                    <p className="text-xl font-black text-green-400">High</p>
                  </div>
                </div>
                <p className="text-[10px] text-white/60 leading-relaxed italic pr-4">
                  "Foundation patterns are now integrated at a neuromuscular level. Moving to specialized hypertrophy protocols."
                </p>
              </div>
