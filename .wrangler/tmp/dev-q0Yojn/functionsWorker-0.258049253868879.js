...
// .wrangler/tmp/pages-QfWvne/functionsWorker-0.258049253868879.mjs
...
__name(getFreeExerciseDB, "getFreeExerciseDB");
__name2(getFreeExerciseDB, "getFreeExerciseDB");
async function onRequestGet(context) {
  try {
    const db = await getFreeExerciseDB();
    const list = (db || []).map((ex) => ({
      id: ex.id,
      name: ex.name,
      ...
      imageUrls: (ex.images || []).map(
        (img) => `/api/proxy-gif?url=${encodeURIComponent(
...
Groq.Files = Files;
function validateGroqEnvironment(apiKey) {
  if (!apiKey) {
    throw new Error("[AI] API key is missing. Skipping LLM generation.");
  }
}
__name(validateGroqEnvironment, "validateGroqEnvironment");
__name2(validateGroqEnvironment, "validateGroqEnvironment");
async function generateCompositionBlueprintViaLLM(apiKey, prefs, exerciseDatabaseSummary, baseURL) {
  validateGroqEnvironment(apiKey);
  const groqOptions = { apiKey };
  ...
  const chatCompletion = await groq.chat.completions.create({
    messages: [
      {
        role: "system",
        content: "You are a fitness routine architect. Output exactly JSON, no markdown, no conversational text."
      },
      {
        role: "user",
        content: prompt
  ...
  try {
    ...
    if (parsed.blueprints && Array.isArray(parsed.blueprints)) {
      ...
      return parsed;
    } else if (parsed.data && parsed.data.blueprints && Array.isArray(parsed.data.blueprints)) {
...
__name(generateCompositionBlueprintViaLLM, "generateCompositionBlueprintViaLLM");
__name2(generateCompositionBlueprintViaLLM, "generateCompositionBlueprintViaLLM");
async function generateRoutineScript(apiKey, exercises, goal, creatorMode, context, baseURL) {
  validateGroqEnvironment(apiKey);
  const groqOptions = { apiKey };
  if (baseURL) groqOptions.baseURL = baseURL;
  const groq = new Groq(groqOptions);
  ...
  const chatCompletion = await groq.chat.completions.create({
    messages: [
      {
        role: "system",
        content: "You are a fitness scriptwriter. Output exactly JSON, no markdown, no conversational text."
      },
      {
        role: "user",
        content: prompt
...
__name(generateRoutineScript, "generateRoutineScript");
__name2(generateRoutineScript, "generateRoutineScript");
async function classifyWorkoutIntentViaLLM(apiKey, promptText, baseURL) {
  validateGroqEnvironment(apiKey);
  const groqOptions = { apiKey };
  if (baseURL) groqOptions.baseURL = baseURL;
  const groq = new Groq(groqOptions);
...
__name(classifyWorkoutIntentViaLLM, "classifyWorkoutIntentViaLLM");
__name2(classifyWorkoutIntentViaLLM, "classifyWorkoutIntentViaLLM");
...
__name(generateAIVideo, "generateAIVideo");
__name2(generateAIVideo, "generateAIVideo");
async function generateSEOMetadata(apiKey, exercises, goal, context, baseURL) {
  validateGroqEnvironment(apiKey);
  const groqOptions = { apiKey };
  if (baseURL) groqOptions.baseURL = baseURL;
  const groq = new Groq(groqOptions);
...
__name(generateSEOMetadata, "generateSEOMetadata");
__name2(generateSEOMetadata, "generateSEOMetadata");
async function generateSocialCaptions(apiKey, exercises, goal, creatorMode, context, baseURL) {
  validateGroqEnvironment(apiKey);
  const groqOptions = { apiKey };
  if (baseURL) groqOptions.baseURL = baseURL;
  const groq = new Groq(groqOptions);
...
__name(generateSocialCaptions, "generateSocialCaptions");
__name2(generateSocialCaptions, "generateSocialCaptions");
...
async function onRequestPost3(context) {
  try {
    const { request, env } = context;
    const body = await request.json();
    const { exercises, goal, context: reqContext } = body;
    const apiKey = env.GROQ_API_KEY;
    if (!apiKey) {
      return errorResponse("GROQ_API_KEY is not configured", 500);
    }
    ...
    return errorResponse(
      error?.message || "Failed to generate SEO metadata",
      500
    );
  }
}
...
var EXERCISE_DB_URLS2 = [
  "https://raw.githubusercontent.com/yuhonas/free-exercise-db/main/dist/exercises.json",
  "https://raw.githubusercontent.com/yuhonas/free-exercise-db/master/dist/exercises.json"
];
var cachedDB2 = null;
async function getFreeExerciseDB2() {
  if (cachedDB2 && cachedDB2.length > 0) return cachedDB2;
  for (const url of EXERCISE_DB_URLS2) {
    try {
      const res = await fetch(url);
      if (res.ok) {
        cachedDB2 = await res.json();
        return cachedDB2;
      }
    } catch {
    }
...
__name(getFreeExerciseDB2, "getFreeExerciseDB2");
__name2(getFreeExerciseDB2, "getFreeExerciseDB");
var synonymMap = {
...
async function onRequestGet2(context) {
  try {
    ...
    const searchVariations = Array.from(/* @__PURE__ */ new Set([
      searchQuery,
      ...synonymMap[searchQuery] || [],
      searchQuery.split(" ")[0]
    ])).filter((t) => t && t.length >= 2);
    const db = await getFreeExerciseDB2();
    let data = [];
    for (const term of searchVariations) {
      ...
      const stretchesBodyOnly = matches.filter(
        (i) => i.equipment === "body only" || i.category === "stretching"
      );
    ...
    const processedData = data.slice(0, 10).map((item) => {
      const rawImageUrl = item.images && item.images.length > 0 ? `https://raw.githubusercontent.com/yuhonas/free-exercise-db/main/exercises/${item.images[0]}` : null;
...
function fallbackParser(prompt) {
...
__name(fallbackParser, "fallbackParser");
__name2(fallbackParser, "fallbackParser");
...
// .wrangler/tmp/bundle-VweZPI/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-VweZPI/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.258049253868879.js.map
